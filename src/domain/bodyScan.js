...
exports.SCAN_METRICS = void 0;
exports.latestScan = latestScan;
exports.scanChange = scanChange;
exports.deriveMissing = deriveMissing;
exports.validateScan = validateScan;
exports.segmentalBalance = segmentalBalance;
const date_1 = require("./date");
const nutrition_1 = require("./nutrition");
/** The measured metrics, in the order a summary should list them. */
exports.SCAN_METRICS = [
...
const delta = (from, to) => from === undefined || to === undefined ? undefined : round(to - from, 2);
...
function scanChange(from, to) {
    return {
        fromDate: from.date,
        toDate: to.date,
        days: (0, date_1.daysBetween)(from.date, to.date),
 ...
 * BMI is left alone.
 */
function deriveMissing(scan, heightCm) {
    const next = { ...scan };
    if (next.weightKg > 0) {
    ...
    if (next.bmi === undefined && heightCm !== undefined && heightCm > 0) {
        next.bmi = (0, nutrition_1.bodyMassIndex)(heightCm, next.weightKg);
    }
    return next;
}
/* ------------------------------------------------------------ validation -- */
...
function segmentProblems(values, what) {
    if (!values)
        return [];
    const problems = [];
...
function validateScan(scan) {
    ...
    for (const metric of exports.SCAN_METRICS) {
    ...
    if (bodyFatPercent !== undefined &&
        ...
        problems.push(`Percent body fat of ${round(bodyFatPercent, 1)}% is outside the readable ` +
            `${MIN_BODY_FAT_PERCENT}-${MAX_BODY_FAT_PERCENT}% range.`);
    }
    if (skeletalMuscleKg !== undefined &&
        fatFreeMassKg !== undefined &&
        skeletalMuscleKg > fatFreeMassKg) {
        problems.push(`Skeletal muscle (${round(skeletalMuscleKg, 1)} kg) cannot be more than ` +
            `fat free mass (${round(fatFreeMassKg, 1)} kg).`);
    }
    if (fatFreeMassKg !== undefined && weightKg > 0 && fatFreeMassKg > weightKg) {
        problems.push(`Fat free mass (${round(fatFreeMassKg, 1)} kg) cannot be more than ` +
    ...
    if (bodyFatKg !== undefined && bodyFatPercent !== undefined && weightKg > 0) {
        ...
        if (expected > 0 && Math.abs(bodyFatKg - expected) / expected > FAT_MASS_TOLERANCE) {
            problems.push(`Body fat mass of ${round(bodyFatKg, 1)} kg does not match ` +
 ...
 */
function segmentalBalance(scan) {
